...
{
	// 初始化 文件字典
	CSJsonDictionary *jsonDict = new CSJsonDictionary();
...
{
    cocos2d::Object* obj = _textureImageDict->objectForKey(name);
    if (obj != nullptr){
        std::string image = dynamic_cast<cocos2d::String*>(obj)->getCString();
        DictElement* de = nullptr;
        CCDICT_FOREACH(_fileDict, de){
            if (de != nullptr){
                cocos2d::String* str = dynamic_cast<String*>(de->getObject());
